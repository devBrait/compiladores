...
typedef enum {
    ...
    ATRIBUICAO,
    IGUALDADE,
    WHILE // Novo átomo adicionado para a palavra reservada
} TAtomo;
...
TInfoAtomo obter_atomo(void) {
    ...
    while (1) {
        ...
        switch (estado) {
            ...
            case 2: // Tratamento de Identificadores e Palavras Reservadas
                if (isalnum(c)) {
                    buffer[pos++] = c;
                    if (pos > 15) {
                        info_atomo.atomo = ERRO;
                    ...
                    // Intercepta a string para checar se é a palavra reservada
                    if (strcmp(buffer, "while") == 0) {
                        info_atomo.atomo = WHILE;
                        info_atomo.linha = linha_atual;
                        strcpy(info_atomo.atributo_ID, ""); // Palavra reservada não precisa do atributo ID
                    } else {
                        // Se não for "while", é um identificador normal
    ...
    while (1) {
        ...
        switch (token.atomo) {
            ...
            case NUMERO:
                ...
                break;
            case WHILE:
                printf("%d# sWHILE | -\n", token.linha); // Imprime conforme o enunciado
                break;
            default:
                break;
        }
